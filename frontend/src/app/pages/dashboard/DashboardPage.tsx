...
interface Property {
  ...
  location: string;
  roomTypes: RoomType[];
  bookings: Record<string, number>; // key: `${roomTypeIndex}-${dayNumber}`, value: rooms booked
}
...
const DashboardPage: React.FC = () => {
  ...
  const renderPropertyTable = (property: Property) => (
    <div key={property.id} className="mb-8 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      {/* Property Title Bar */}
        ...
        <table className="w-full text-xs border-collapse">
          ...
          <tbody>
            ...
            {property.roomTypes.map((rt, rtIdx) => (
              <tr key={rtIdx} className="hover:bg-slate-50 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-0.5 text-sm font-medium">
            Monthly occupancy grid across all properties
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-white border border-slate-300 rounded-lg px-3 py-2 shadow-sm">
          <button
            onClick={prevMonth}
            className="p-1 rounded hover:bg-slate-100 transition-colors text-slate-600"
